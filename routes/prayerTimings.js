...
const SUNRISE_API = process.env.SUNRISE_SUNSET_API;
const NAMAZ_TIMINGS_URL = process.env.NAMAZ_TIMINGS_URL;
...
async function fetchSunriseSunset(now, fetch) {
  ...
  const url = `${SUNRISE_API}?lat=${LATITUDE}&lng=${LONGITUDE}&date=${isoDate}&formatted=0`;
...
router.get('/', async (req, res) => {
  try {
    ...
    const htmlResp = await fetch(NAMAZ_TIMINGS_URL);
